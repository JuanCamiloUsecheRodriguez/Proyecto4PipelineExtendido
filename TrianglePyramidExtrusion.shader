			...
			Pass
			{
				...
				void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream)
				{
					...
					for (int i = 0; i < 3; i++) {
						...
						o.vertex = UnityObjectToClipPos(barycenter + float4(normal, 0.0) * ((sin(3 * _Time.y)*0.3)+ 0.3));
						UNITY_TRANSFER_FOG(o,o.vertex);
						o.uv = TRANSFORM_TEX(IN[i].uv, _MainTex);
						o.color = fixed4(((sin(3 * _Time.y )*0.3)+0.3), ((sin(2* _Time.y)*0.3) + 0.3), ((sin(4 * _Time.y)*0.3) + 0.3), ((sin(_Time.y)*0.3) + 0.3));
						triStream.Append(o);
